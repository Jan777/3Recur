...
public class JugadorMP extends Grupo {
	...
	private PC pj;
	private int color;
	public JugadorMP(PC pj,Nivel nivel)
	{
	...
	public void renderizar(Pantalla pantalla) {
		...
		pantalla.render(xOffset + modificador - (modificador * flipBottom), yOffset + modificador,
				(xTile + 1) + (yTile + 1) * 32, color, flipBottom, escala);
		// renderizamos el nombre de usuario

		if (pj.getName() != null) {
			Fuente.render(pj.getName(), pantalla, xOffset - ((pj.getName().length() - 1) / 2 * 8), yOffset - 10,
					Color.get(-1, -1, -1, 555), 1);
		}
	}
	...
	public boolean colision(int xa, int ya) {
		...
		int yMin = 3;
		int yMax = 7;
		// ciclos para recorrer las cuatro direcciones del sector del sprite que
		// va a ser caja colision
